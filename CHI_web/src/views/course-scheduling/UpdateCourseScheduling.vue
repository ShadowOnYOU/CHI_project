<template>
  <el-dialog title="修改" width="600px"
    ...
    <el-form ref="form" :model="form" :rules="rules" inline label-width="80px" class="tams-form-container">
      <el-form-item label="课程" prop="courseId">
        <el-select v-model="currentCourse" value-key="id" class="tams-form-item" @change="courseChange" disabled>
          <el-option v-for="item in courseData" :key="item.id" :label="item.name" :value="item">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="老师" prop="teacherId">
        <el-select v-model="form.teacherId" class="tams-form-item"  disabled>
      ...
      <el-form-item label="教室" prop="classroomId">
        <el-select v-model="form.classroomName" class="tams-form-item">
...
export default {
  ...
  methods: {
    ...mapActions(['GetClassroomRefList', 'GetClassroomById', 'GetCourseRefList', 'GetTeacherRefList', 'GetCourseSchedulingById', 'UpdateCourseSchedulingById']),
    init () {
      this.attendTimeSelectKey = new Date().getTime()
      Promise.all([
        this.GetClassroomById(this.$store.state.selectedId),
    ...
    search () {
      this.GetCourseSchedulingById(this.id).then(res => {
        if (res) {
          this.form = res
          this.courseDuration = moment(this.form.finishTime, 'HH:mm:ss').diff(moment(this.form.attendTime, 'HH:mm:ss'), 'minutes')
          this.form.attendTime = moment(this.form.attendTime, 'HH:mm:ss').format('HH:mm')
          console.log(this.form.attendTime)
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          const closestStartTime = availableStartTimes.reduce((prev, curr) => {
            const prevDiff = Math.abs(moment(prev, 'HH:mm:ss').diff(moment(this.form.attendTime, 'HH:mm:ss')))
            const currDiff = Math.abs(moment(curr, 'HH:mm:ss').diff(moment(this.form.attendTime, 'HH:mm:ss')))
            return currDiff <= prevDiff ? curr : prev
          })
          const timeInterval = moment(this.form.finishTime, 'HH:mm:ss').diff(moment(this.form.attendTime, 'HH:mm:ss'))
          this.form.attendTime = closestStartTime
          this.form.finishTime = moment(this.form.attendTime, 'HH:mm:ss').add(timeInterval).format('HH:mm:ss')
